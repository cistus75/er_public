...
누적 활성 사용자 **2.6만 명** 규모의 서비스를 2인 팀으로 개발·운영했습니다. 본 저장소는 운영 종료 후 테스트 및 포트폴리오 용도로 정리한 버전입니다.

## 핵심 성과

- GA4 기준 활성 사용자 **2.6만 명**, 이벤트 **48만 회**, 활성 사용자당 평균 참여 시간 **2분 36초**
- FastAPI 기반 REST API와 Gemini API 기반 개인화 피드백 기능 개발
- 외부 게임 API 변경에 맞춰 `userId` 기반에서 > `gameId` 기반 데이터 수집기로 재설계
...
## 1. 관심사 분리를 통한 백엔드 구조 개선

- **문제:** `main.py`에 기능이 집중되면서 파일이 비대해지고, 팀 작업 중 병합 충돌과 롤백이 반복됐습니다.
- **해결:** 백엔드를 `routers`, `services`, `core`, `db`, `common`, `exceptions`로 나누고, `main.py`는 애플리케이션 설정과 모듈 연결만 담당하도록 재구성했습니다.
- **결과:** 기능별 책임과 변경 범위를 분리해 공통 파일을 동시에 수정하는 작업과 유지보수 부담을 줄였습니다.

## 2. 운영·테스트 환경 분리

- **문제:** 라이브 환경에서 신규 기능 배포와 버그 수정 테스트를 진행하면서 사용자에게 버그가 노출되거나 서비스가 일시적으로 중단되는 문제가 발생했습니다.
- **해결:** 운영 환경과 별도의 테스트 환경을 구축하고, 테스트 환경에서 주요 기능을 확인한 코드만 운영 환경에 배포하도록 절차를 변경했습니다.
- **결과:** 변경 사항을 운영 배포 전에 검증할 수 있게 됐으며, 라이브 환경에서 직접 테스트하는 빈도를 줄였습니다.

## 3. 외부 API 변경에 따른 데이터 수집기 재설계

- **문제:** 외부 게임 API의 사용자 식별 방식이 변경되면서, 특정 사용자 목록을 따라 전적을 수집하던 기존 로직이 정상적으로 동작하지 않았습니다.
- **해결:** 수집 기준을 사용자에서 게임으로 전환하고, 연속적으로 증가하는 `gameId`를 활용해 최신 게임부터 역순으로 탐색하도록 재설계했습니다. 요청량은 limiter로 제어하고, 게임 날짜를 종료 조건으로 사용했으며, `gameId` 기준 upsert를 적용했습니다.
- **결과:** 특정 사용자 목록에 의존하지 않고 일정 기간의 최근 게임 데이터를 수집할 수 있게 됐으며, 동일한 게임 데이터의 중복 저장을 방지했습니다. 또한 기존 방식의 문제였던 티어 편향을 해소했습니다.

## 4. AI API 요청 실패 대응

- **문제:** 사용자 증가와 AI 모델 마이그레이션 과정에서 요청 한도 초과, 일시적인 서버 오류, 네트워크 타임아웃이 발생했으며, 일부 AI 요청 실패가 전체 응답에 영향을 주는 문제가 있었습니다.
- **해결:** AI API 호출을 별도의 서비스 모듈로 분리하고, 세마포어로 동시 요청 수를 제한했습니다. 오류 유형을 기록하고 재시도 가능한 오류에는 제한된 재시도를 적용했으며, 각 AI 분석 작업의 예외를 독립적으로 처리했습니다.
- **결과:** 일부 AI 분석이 실패하더라도 전적과 통계 데이터는 제공하고, 실패한 항목에만 일시적인 오류 안내를 표시할 수 있도록 변경했습니다.

## 5. 제한된 예산에서의 서비스 운영

- **문제:** 별도의 수익원 없이 웹 서버, 데이터베이스, AI API를 지속적으로 운영해야 했습니다.
- **해결:** Vercel, Render, MongoDB의 무료 사용 범위와 Google 프로모션 크레딧을 활용해 운영 비용을 줄였습니다.
- **결과:** 별도의 고정 인프라 비용 없이 약 10개월간 서비스를 운영했습니다. 이후 기존 AI 모델의 지원 종료에 따른 마이그레이션과 유지보수 부담을 고려해 서비스를 종료했습니다.

## 6. AI 응답 지연에 따른 대기 UX 개선

- **문제:** 외부 API 조회와 AI 분석 생성 시간이 길어지면서 사용자가 현재 진행 상태를 확인하기 어려웠습니다.
- **해결:** 단계별 처리 시간을 기록해 AI 분석 생성 구간의 비중이 크다는 점을 확인했습니다. 지표를 먼저 표시하는 방식도 적용했지만 추가 로딩으로 화면 흐름이 분절돼 되돌렸으며, 대신 로딩 화면에 진행 상태와 게임 관련 콘텐츠를 제공했습니다.
- **결과:** 사용자가 대기 중 현재 진행 상태와 부가 콘텐츠를 확인할 수 있도록 개선했으며, 단순한 화면 선노출보다 로딩 전후의 일관된 흐름이 중요하다는 점을 확인했습니다.
